...
#define DEBUG_TYPE "cdacli"

STATISTIC(LoopsInterchanged, "Number of loops interchanged");

static cl::opt<int> CdacLICostThreshold(
    "cdacli-threshold", cl::init(0), cl::Hidden,
...
struct LoopInterchange {
  ...
  bool run(LoopNest &LN) {
    ...
    if(LoopList.size()<3) return false;//added
...
LoopInterchangeProfitability::isProfitablePerInstrOrderCost() {
  ...
  if (Cost < 0 && Cost < CdacLICostThreshold)
    return std::optional<bool>(true);
...
void LoopInterchangeTransform::restructureLoops(
  ...
  NewInner->removeBlockFromLoop(
      OrigInnerPreHeader); // problem lag rahi h idhr//Theek hai
  LI->changeLoopFor(OrigInnerPreHeader,
                    OuterLoopParent); // problem lag rahi h idhr, 2n argu should
                                      // be a Loop//Theek hai
  ...
  while (!NewOuter->isInnermost()) // problem lag rahi h idhr, this while will
                                   // only stop when this NewOuter loop becomes
                                   // the Innermost Loop
    NewInner->addChildLoop(
        NewOuter->removeChildLoop(NewOuter->begin())); // kya bakwas h ye!
  ...
  for (BasicBlock *BB : NewInner->blocks())
    if (LI->getLoopFor(BB) == NewInner) // this LI->getLoopFor(BB) rturns the
                                        // Innermost loop that contains the BB
      NewOuter->addBlockEntry(BB); // how outer bbs is inserted into the inner
                                   // when NewOuter is already the outer loop
  ...
  BasicBlock *OuterHeader =
      NewOuter->getHeader(); // problem lag rahi h idhr; why are we removing
                             // header and latch from new outer loop
...
bool LoopInterchangeTransform::transform() {
  ...
  if (InnerLoop->getSubLoops().empty()) {
    ...
    if (CondI) {
      LLVM_DEBUG(
          dbgs()
          << "Inserting the termination cond as instruction to the worklist\n");
      WorkList.insert(CondI);
    }
    LLVM_DEBUG(dbgs() << "WorkList size now is " << WorkList.size()
                      << " and now MoveInstruction\n");
    MoveInstructions();
    for (Instruction *InnerIndexVar : InnerIndexVarList) {
      LLVM_DEBUG(
          dbgs() << "Inserting values from InnerIndexVarList to workList\n");
      WorkList.insert(cast<Instruction>(InnerIndexVar));
    }
    LLVM_DEBUG(dbgs() << "WorkList size now is " << WorkList.size() << " \n");
    LLVM_DEBUG(dbgs() << "Now again MoveInstruction\n");
    MoveInstructions();
    LLVM_DEBUG(dbgs() << "The current size of the worklist after completion of "
                         "the MoveInstruction is "
                      << WorkList.size() << "\n");
  }
...
PreservedAnalyses CdacLIPass::run(LoopNest &LN,
