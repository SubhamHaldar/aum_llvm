...
static cl::opt<bool> EnableInterleavedMemAccesses(
    "enable-interleaved-mem-accesses", cl::init(true), cl::Hidden, //adm
    cl::desc("Enable vectorization on interleaved memory accesses in a loop"));

/// An interleave-group may need masking if it resides in a block that needs
/// predication, or in order to mask away gaps.
static cl::opt<bool> EnableMaskedInterleavedMemAccesses(
    "enable-masked-interleaved-mem-accesses", cl::init(true), cl::Hidden, //adm
...
static cl::opt<unsigned> ForceTargetMaxScalarInterleaveFactor(
    "force-target-max-scalar-interleave", cl::init(2), cl::Hidden, //adm
    cl::desc("A flag that overrides the target's max interleave factor for "
             "scalar loops."));

static cl::opt<unsigned> ForceTargetMaxVectorInterleaveFactor(
    "force-target-max-vector-interleave", cl::init(2), cl::Hidden, //adm
    cl::desc("A flag that overrides the target's max interleave factor for "
             "vectorized loops."));
...
void InnerLoopVectorizer::fixNonInductionPHIs(VPlan &Plan,
  ...
  for (VPBasicBlock *VPBB : VPBlockUtils::blocksOnly<VPBasicBlock>(Iter)) {
    for (VPRecipeBase &P : VPBB->phis()) {
     ...
     for (unsigned Part = 0, UF = State.UF; Part < UF; ++Part) { //adm added loop
        PHINode *NewPhi = cast<PHINode>(State.get(VPPhi, Part)); //adm 
      ...
      for (unsigned i = 0; i < VPPhi->getNumOperands(); ++i) {
        ...
        NewPhi->addIncoming(State.get(Inc, Part), State.CFG.VPBB2IRBB[VPBB]);//adm 0->Part
       }
      }
    }
  }
...
static bool processLoopInVPlanNativePath(
...

  {
    ...
    InnerLoopVectorizer LB(L, PSE, LI, DT, TLI, TTI, AC, ORE, VF.Width,
                           VF.Width, UF, LVL, &CM, BFI, PSI, Checks); // adm 1->UF
    LLVM_DEBUG(dbgs() << "Vectorizing outer loop in \""
                      << L->getHeader()->getParent()->getName() << "\"\n");
    LVP.executePlan(VF.Width, UF, BestPlan, LB, DT, false); //adm 1->UF
  }
